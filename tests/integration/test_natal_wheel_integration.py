...
def test_placidus_houses():
    ...
    try:
        houses = calc.calculate_placidus_houses(
            test_date, test_time, test_location, test_timezone
        )
        ...
        for i in range(1, 13):
            if i in houses:
                house = houses[i]
                print(
                    f'  House {i}: {house["sign"]} {house["cusp"]} (lon: {house["longitude"]})'
                )
            else:
...
def test_equal_houses():
    ...
    try:
        houses = calc._calculate_equal_houses(
            test_date, test_time, test_location, test_timezone
        )
...
def test_natal_wheel_data():
    ...
    try:
        wheel_data = calc.calculate_natal_wheel_data(
            test_date, test_time, test_location, test_timezone
        )
...
def test_javascript_files():
    ...
    js_files = ["static/js/components/wheel/d3-wheel-renderer.js"]
...
def test_template_files():
    ...
    template_files = ["templates/natal/wheel.html"]
...
def test_api_endpoint():
    ...
    try:
        ...
        if (
            "path('natal/wheel/', NatalWheelView.as_view()" in content
            or "NatalWheelView" in content
        ):
