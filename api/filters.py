...
class DailyTransitFilter(filters.FilterSet):
    ...
    transit_date_after = filters.DateFilter(
        field_name="transit_date", lookup_expr="gte"
    )
    transit_date_before = filters.DateFilter(
        field_name="transit_date", lookup_expr="lte"
    )
    g_code_score_min = filters.NumberFilter(
        field_name="g_code_score", lookup_expr="gte"
    )
    g_code_score_max = filters.NumberFilter(
        field_name="g_code_score", lookup_expr="lte"
    )
...
class GeneratedContentFilter(filters.FilterSet):
    ...
    generated_after = filters.DateTimeFilter(
        field_name="generated_at", lookup_expr="gte"
    )
    generated_before = filters.DateTimeFilter(
